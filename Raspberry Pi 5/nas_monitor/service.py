...
class CollectorService:
    ...
    def start(self):
        ...
        ambient = find_ambient_sensor(self.config.sensors)
        if ambient:
            jobs.append(("ambient.temperature_c", "celsius", self.config.collection.temperature_interval_seconds,
                         lambda: collect_ambient_temperature(ambient)))
        else: LOGGER.warning("No DS18B20 ambient temperature sensor found")
        for job in jobs:
