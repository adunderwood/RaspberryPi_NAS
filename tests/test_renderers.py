...
from nas_display.renderers.cpu_memory import CpuMemoryRenderer

def test_renderers_produce_panel_sized_palette_images():
    snapshot = {"cpu":{"usage_percent":25,"temperature_c":50,"usage_history":[{"value":10},{"value":50}]},
                "memory":{"usage_percent":62.5,"usage_history":[{"value":60},{"value":62.5}]},
                ...
                "degraded_drives":0,"drives":[
                    {"device":f"/dev/sd{letter}","state":"in_sync","errors":0,"healthy":True,"temperature_c":30+index}
                    for index,letter in enumerate("abcd")]}]}}
    policy = {"theme":"dark","temperature_unit":"F","thresholds":{"storage_percent":90}}
    for size in ((212, 104), (250, 122)):
        for renderer in (OverviewRenderer(), StorageRenderer(), ThermalRenderer(), DriveHealthRenderer(), CpuMemoryRenderer()):
...
def test_renderers_preserve_original_panel_safe_area():
    snapshot = {"cpu":{"usage_percent":25,"temperature_c":50,"usage_history":[{"value":25}]},
    ...
    renderers = (OverviewRenderer(), StorageRenderer(), ThermalRenderer(), DriveHealthRenderer(), CpuMemoryRenderer(), OfflineRenderer())
    for size in ((212, 104), (250, 122)):
